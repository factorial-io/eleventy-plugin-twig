 ...
 * @param  {...Array<string>} imageOptions
 * @returns {string}
 */
module.exports = (eleventyConfig, userOptions, ...[filename, alt]) => {
  ...
  if (!userOptions.images?.widths || !userOptions.images?.widths.length) {
    throw new Error(`userOptions.images.widths should include at least one width (for example [300, 600])`);
  }
  if (!userOptions.images?.formats || !userOptions.images?.formats.length) {
    throw new Error(`userOptions.images.formats should include at least one format (for example ["webp", "avif"])`);
  }

  const options = {
    widths: userOptions.images.widths,
    formats: userOptions.images.formats,
    outputDir: path.join(userOptions.dir.output, userOptions.assets.base, userOptions.assets.images),
    urlPath: path.join(userOptions.assets.base, userOptions.assets.images),
  };
  const currentPath = path.join(userOptions.assets.root, userOptions.assets.base, userOptions.assets.images, filename);
  ...
  const imageElement = `<img src="${lowsrc.url}" width="${highsrc.width}" height="${highsrc.height}" alt="${alt}" ${userOptions.images.additionalAttributes}>`;
