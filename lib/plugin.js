...
const handleErrors = (userOptions) => {
  ...
  /**
   * fine to use typeof here because its a value from the config
   * object on not object-wrapped with new String()
   */
  if (
    typeof userOptions.mixManifest === "string" &&
    userOptions.mixManifest?.length === 0
  ) {
    console.error(
      "userOptions.mixManifest is defined but doesnt contain a valid filename ('foobar.json')"
    );
  }

  if (userOptions.mixManifest && !userOptions.assetsPath) {
    console.error(
      "userOptions.mixManifest requires userOptions.assetsPath.{base, js, css}"
    );
  }

  if (!userOptions.dir) {
    console.error("Missing userOptions.dir.");
  }

  if (userOptions.dir && !userOptions.dir.output) {
    console.error("userOptions.dir requires userOptions.dir.output");
  }
};
