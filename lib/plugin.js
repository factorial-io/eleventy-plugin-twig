...
const handleErrors = (userOptions) => {
  ...
  if (
    ...
    errors.push(
      "userOptions.mixManifest is defined but doesnt contain a valid filename (for example 'foobar.json')"
    );
  }

  if (userOptions.mixManifest && !userOptions.assetsPath?.base) {
    errors.push("userOptions.mixManifest requires userOptions.assetsPath.base");
  }

  if (!userOptions.dir) {
    errors.push("Missing userOptions.dir.");
  }

  if (userOptions.dir && !userOptions.dir.output) {
    errors.push("userOptions.dir requires userOptions.dir.output");
  }
