...
const defaultShortcodes = [
  {
    ...
    callback: (eleventyConfig, userOptions, originalAsset) =>
      mixAssets(userOptions.dir.output, userOptions.mixManifest, originalAsset),
    requiredOptions: ["mixManifest"],
  },
  {
    symbol: "asset_path",
    callback: (eleventyConfig, userOptions) =>
      `${userOptions.dir.output}/${userOptions.assetsPath.base}`,
    requiredOptions: ["assetsPath"],
  },
  {
    symbol: "image",
    callback: (eleventyConfig, userOptions, imageSrc, imageAlt) =>
...
const extendTwig = (eleventyConfig, userOptions, shortcode) => {
  ...
  const hasRequiredOptions =
    shortcode.requiredOptions.filter(
      (requiredOption) => !availableOptions.includes(requiredOption)
    ).length === 0;
